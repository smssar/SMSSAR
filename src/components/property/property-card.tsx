...
export default function PropertyCard({
  ...
  const propertyTypeLabel = (() => {
    ...
    if (locale === "ar") {
      return property.propertyType.name_ar || property.propertyType.name;
    }

    if (locale === "fr") {
      return property.propertyType.name_fr || property.propertyType.name;
    }

    return property.propertyType.name;
  })();
  const sellerLabel = (() => {
    if (typeof property.seller === "string") {
    ...
    return property.seller.name || property.seller.email || "";
  })();
  const rentLabel =
      ...
      <Card className="group overflow-hidden border-border/70 bg-card transition hover:-translate-y-1 hover:shadow-2xl">
          ...
          <div className="absolute left-4 top-4 flex max-w-[calc(100%-2rem)] flex-wrap items-center gap-2">
            {computedAdCount ? (
              <Badge
                variant="secondary"
                aria-label={adLabel}
                className="relative flex items-center gap-2 rounded-full px-3 py-1.5 bg-background font-semibold uppercase tracking-[0.12em] shadow-2xl shadow-violet-600/30 transform transition-all duration-300 hover:scale-105"
              >
        ...
        <CardContent className="space-y-5 p-6">
          ...
          <div className="flex items-center justify-between rounded-2xl bg-muted/40 px-4 py-3">
            ...
            <div className="text-sm text-muted-foreground">
              <div className="font-medium text-foreground">{sellerLabel}</div>
        ...
        <CardFooter className="flex items-center gap-3 p-6 pt-0">
          ...
          {/* <button
            ...
            className="inline-flex h-11 items-center justify-center rounded-md border border-border/70 px-4 text-sm font-medium transition hover:bg-muted"
          >
            Quick View
          </button> */}
          <FavoriteButton
